...
class MainController extends Controller {
  async index() {
    //index方法
    this.ctx.body =
      "<h1 style='text-align: center;  font-size: 10vw'>hi xyl_blog's api</h1>"; // 只是为了只输入网址不出现404
  }
  ...
  async checkAccount() { // 检验旧账户密码是否正确
    let userName = this.ctx.request.body.userName;
    let password = this.ctx.request.body.password;
    console.log(userName, "+++++", password);
    const sql =
    ...
    if (res.length > 0) {
      //表示找到数据，旧账号是正确的
      console.log("旧账号正确，查验成功");
      this.ctx.body = { data: "查验成功" };
    } else {
      this.ctx.body = { data: "旧账号错误" };
    }
  }

  async fixAccount() { //修改账户密码
    let tempAccount = {...this.ctx.request.body, id: 1};
    console.log(tempAccount,'+++++修改密码++++++')
    const result = await this.app.mysql.update("admin_user", tempAccount);
